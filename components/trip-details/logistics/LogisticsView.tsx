...
interface LogisticsViewProps {
    ...
    // Accommodation state
    accommodationFilter: 'all' | 'hotel' | 'home';
    setAccommodationFilter: (filter: 'all' | 'hotel' | 'home') => void;
    accommodationViewMode: 'list' | 'grid';
    setAccommodationViewMode: (mode: 'list' | 'grid') => void;
    accommodationSortOrder: 'newest' | 'oldest';
    setAccommodationSortOrder: (order: 'newest' | 'oldest') => void;
...
const LogisticsView: React.FC<LogisticsViewProps> = ({
    ...
    setAccommodationViewMode,
    accommodationSortOrder,
    setAccommodationSortOrder,
    isLoadingHotels,
    isLoadingTransports,
}) => {
    const [activeSection, setActiveSection] = useState<'accommodation' | 'transport'>('accommodation');

    // Filter and sort hotels
    let displayedHotels = accommodationFilter === 'all'
        ? hotels
        : hotels.filter(h => h.type === accommodationFilter || (!h.type && accommodationFilter === 'hotel'));
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center bg-gray-50/50 p-1.5 rounded-2xl border border-gray-100">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto justify-end px-1">
                    {/* Sort */}
                    <button
                        onClick={() => setAccommodationSortOrder(accommodationSortOrder === 'newest' ? 'oldest' : 'newest')}
                    ...
                    <div className="relative bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-3xl p-16 border border-indigo-100/50 text-center overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="size-24 mx-auto mb-6 bg-white rounded-3xl shadow-lg flex items-center justify-center ring-4 ring-indigo-100/50">
                                <span className="material-symbols-outlined text-5xl text-indigo-500">hotel</span>
                            </div>
                            <h3 className="text-2xl font-bold text-text-main mb-3">Nenhuma hospedagem cadastrada</h3>
                            <p className="text-base text-text-muted mb-8 max-w-lg mx-auto leading-relaxed">
                                Adicione hotéis, Airbnbs ou outras acomodações. Organize suas reservas, datas de check-in/out e mantenha tudo no mesmo lugar.
                            </p>
                            <button
                                onClick={onAddAccommodation}
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeSection === 'accommodation'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeSection === 'transport'
                        ? 'bg-primary text-text-main shadow-md'
                        : 'bg-white text-text-muted hover:bg-gray-100 border border-gray-200'
                        }`}
                >
